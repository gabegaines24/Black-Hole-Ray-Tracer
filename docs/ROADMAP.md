...
| **Schwarzschild kernel** | 2D then 3D RHS + loop matching Python | Not started |
| **`bridge/`** | pybind11 exposing batch trace | Not started |

**Acceptance**

- `make -C kernel` builds `harmonic_demo` where `cc`/`gcc`/`clang` exists; full `pytest` run passes (kernel test skipped without a compiler).
- Schwarzschild parity + bridge build remain **TBD**.
...
- [ ] Schwarzschild geodesic integration in C + parity vs `phase1` / `phase2` Python.
